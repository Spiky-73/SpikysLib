...
public static class TypeHelper {
    ...
    public static MemberInfo GetMember(LambdaExpression expr) => expr.Body switch {
        NewExpression e => e.Constructor,
        MemberExpression e => e.Member,
        MethodCallExpression e => e.Method,
        UnaryExpression e => ((e.Operand as MethodCallExpression)?.Object as ConstantExpression)?.Value as MethodInfo,
        _ => null,
    } ?? throw new ArgumentException("Invalid lambda expression " + expr.Body);
    public static FieldInfo GetField(LambdaExpression expr) => (FieldInfo)GetMember(expr);
    public static PropertyInfo GetProperty(LambdaExpression expr) => (PropertyInfo)GetMember(expr);
    public static MethodInfo GetMethod(LambdaExpression expr) => (MethodInfo)GetMember(expr);
    public static ConstructorInfo GetConstructor(LambdaExpression expr) => (ConstructorInfo)GetMember(expr);
